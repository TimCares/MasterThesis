...
from dictionary import dictionary_utils
from dictionary.file_chunker_utils import Chunker, find_offsets
from dictionary.path_manager import PathManager
...
class Dictionary:
    ...
    def string(
        ...
        sent = separator.join(
            token_string(i)
            for i in tensor
            if dictionary_utils.item(i) not in extra_symbols_to_ignore
        )

        return dictionary_utils.post_process(sent, bpe_symbol)
    ...
    def encode_line(
        self,
        line,
        line_tokenizer=dictionary_utils.tokenize_line,
