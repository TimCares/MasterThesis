...
class AMMData2VecPreTrainingLightningModule(L.LightningModule):
    ...
    def _handle_clone_vg(self, batch:Dict[str, Any]):
        text_keys = [k for k in batch.keys() if k.startswith('text')]
        n_clone = len(text_keys)
        captions = []
        for key in text_keys:
            captions.append(batch[key])
            del batch[key]
        batch['text'] = torch.cat(captions, dim=0)
        batch['n_clone'] = n_clone
        return batch
        

    def _training_step(self, batch:Dict[str, Any], batch_idx):
        if 'target' in batch:
            batch.pop('target') # unused, layer activations are the targets

        if sum(k for k in batch.keys() if k.startswith('text')) > 1:
            batch = self._handle_clone_vg(batch)
        else:
            text = batch['text']
        ...
        kd_losses = []
        kd_pred = prepare_output(output_dict_text['layer_results'], modality)
        if "n_clone" in batch:
            kd_pred = kd_pred.repeat(batch['n_clone'], 1, 1)
        kd_loss = self.kd_loss(input=kd_pred, target=kd_target)
        kd_losses.append(kd_loss)

        kd_pred = prepare_output(output_dict_image['layer_results'], modality)
        kd_loss = self.kd_loss(input=kd_pred, target=kd_target)
        kd_losses.append(kd_loss)
        
        loss = sum(kd_losses) / 2
        
        text_features = output_dict_text['layer_results'][-1].mean(dim=1)
        image_features = output_dict_image['layer_results'][-1].mean(dim=1)

        self.log(f"train/kd_mse", loss, prog_bar=True)
