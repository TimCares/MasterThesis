...
This has three advantages: First, labels do not need to be created by hand,
it is straightforward to randomly mask words in a sentence and use them as targets during training.
Second, massive amounts of text available on the internet enable the generation of massive training datasets.
Last, and most important, the model learns to understand and represent the world we live in.
...
gained from self-supervision can be transferred to downstream tasks. For instance, if a model has learned
...
interpretable representations of the respective input data, these models often lack the ability to understand that real-world
...
but cannot process the text. While a text model can generate a representation of the text, this representations will not be
the same as the one generated by the image (from the image model). This is because, although
both modalities express the same concept, the models have been trained independently 
...
by researchers from OpenAI @neural_scaling_law (seen in @neural_scaling_law_fig).
