...
by Aytar et al. @shre. For simplicity, this paper will be referred to as “SHRe”
...
(also the Transformer architecture).
...
not specified what exact model was used. The training objective is to minimize the KL-Divergence between the teacher and student model.
...
layer (the last linear layer of the shared encoder outputs the probability distribution over ImageNet-1K classes). 
...
They can vary, as long as the resulting probability distribution of the last linear layer
is the same as the teacher's output.
...
Different from contrastive loss, for a given input, e.g. an image, the ranking loss does not normalize the similarity score
...
#figure(
  ...
  and is splitted into 5 chunks of 1k samples each. Retrieval is then performed only between the samples inside of each chunk,
  and the metric used is
  the median rank. The median rank denotes the position of the correct pair in the list of possible retrieval candidates (1k candidates),
  the lowest rank is 1, meaning that the correct pair has the highest similarity score to the query.
  The median rank is averaged over all chunks for each datasets, so the results
...
from a *supervised* unimodal (image) model can be _extracted_ and used to train a multimodal model.
