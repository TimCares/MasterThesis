...
A viable strategy would be to only use a few layers from the pretrained model, but since the environment in
which those layers were trained is different from the one in which they are used during finetuning, this
approach requires longer training times to adapt to the new environment.
...
Let $f(dot)$ be the teacher model, $g(dot)$ the student model, and $bold(x)$ the input sample of any modalitiy, e.g. an image or a text.
...
#figure(
  ...
  caption: [
    ...
    from scratch on a classification taks, while the model prediction over the classes can be used for knowledge distillation.
...
computed by the KL-Divergence (introduced in (TODO:cite)).

Consequently, recalling the definition of the KL-Divergence in (TODO: cite), the training objective of response-based knowledge distillation
is to bring the probability distributions of the student model, for a given sample
(or all samples in general), as close as possible to the probability distributions of the teacher model for the same sample(s). 
...
the student model does not necessarily have to only predict the final output (probability distribution) of the teacher model.

The activations of the teacher model are usually regressed using the Mean Squared Error (MSE) as the loss function (defined in (TODO: cite)). The
...
of the teacher 1:1. In this case, other strategies have to be used, and we will introduce one of them in (TODO: cite unimodal kd experiments).
