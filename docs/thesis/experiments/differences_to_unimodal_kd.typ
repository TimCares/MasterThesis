...
For a single image-text pair, the image can be passed to the teacher, and the image and its caption can be passed to the student separately.
...
Concretely, the representation of the $mono(["I_CLS"])$ token should *not* contain any image-specific information, as this would make it impossible
...
the teacher, which is the global image representation, is generated by the student's classification head. A consequence of this is that
...
as shown in @image_text_retrieval_S_SMKE. We achieve competitive performance with FLAVA @flava and CLIP @clip on COCO, and
