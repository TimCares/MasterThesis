...
regress the teacher's representation $bold(h)^t_(v, L_t, mono(["I_CLS"]))$ of the $mono(["I_CLS"])$ token (see @kd_loss_mse). We found that requiring
...
Usually, this would actually be a good idea, as it would push both representations closer together
...
$bold(h)^t_(v, L_t, mono(["I_CLS"]))$ than the caption representation $bold(h)'''^s_(w, K, mono(["T_CLS"]))$,
...
If the teacher's representation $bold(h)^t_(v, L_t, mono(["I_CLS"]))$ would not contain any image-specific information, so for example
would encode the information "dog" for an image of a dog, the student should be able to regress this representation equally well. However,
