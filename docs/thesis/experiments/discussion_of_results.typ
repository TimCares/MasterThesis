...
We compare the performance on natural language processing, computer vision,
...
was designed to be a *proof-of-concept* for the feasibility of creating cheap and efficient vision-language models.
Since we achieve reasonable performance across all tasks, outperforming well-known baselines on tasks like WNLI
...
- We outperform BERT @bert and DistilBERT @distilbert (by just 0.1 percentage points) on the WNLI task, which is likely
...
- Our specialized variant of the image-only distillation, C-DistilData2Vec2, is almost on par with BEiTv2 @beitv2
  on CIFAR-10 and CIFAR-100 @cifar_10_100 linear evaluation. This is remarkable, as BEiTv2 is also twice as large as
  our model. We consider the results on both benchmarks (CIFAR-10 and CIFAR-100) as reliable, as we perform the linear
  evaluation of BEiTv2 ourselves, with the exact same setup as for C-DistilData2Vec2.
- S-SMKE outperforms CLIP @clip on COCO image retrieval, and the finetuned variant of S-SMKE outperforms CLIP
  on all COCO retrieval metrics. It has to be noted that CLIP was neither pretrained nor finetuned on the COCO dataset,
  so S-SMKE has an advantage here. However, since CLIP is a much larger model that was trained with >121$times$ the data,
...
can greatly improve performance when finetuning on text tasks. The same can be observed for BEiT-3 @beit3 on vision tasks
...
F-DistilBERT reaches 55.1%, which is just 1.2 percentage points below the 56.3% of BERT @bert. Unfortunately, we do not have
an explanation for this discrepancy. The only factor that could have caused this is the fact that we pretrain S-SMKE
on subsets of Conceptual Captions 3M and 12M @cc3m @cc12m. Since those datasets are largely uncurated and scraped from the web,
the captions, which are passed through the text encoder, might not be grammatically correct. Since CoLA is a task to
check the grammatical understanding of a model @cola, finetuning on a dataset with just 8551 samples might not be enough
