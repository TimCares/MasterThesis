...
$ <transformer_shre_shared_encoder_computation>
...
We hypothesize that our approach might be fore challenging to learn, compared to VLMo and BEiT-3's concatenated strategy,
...
Nevertheless, VLMo demonstrates that this modality-specific complexity may not be an insurmountable issue.
In VLMo, masked language modeling is performed using a Transformer whose Self-Attention weights are frozen
and pre-trained on images, yet the model still effectively handles text-only pretraining @vlmo.
...
Recall that we implemented the 3-layer MLP as the 2-layer MLP network
...
#figure(
  ...
  caption: [
    ...
    Please not that the shared Transformer layer we implement is a normal Transformer layer, i.e. it includes the two residual connections
    and the layer normalizations. However, we do not show them in the illustration for simplicity.
],
